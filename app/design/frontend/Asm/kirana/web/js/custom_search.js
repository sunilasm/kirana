...
"jquery/jquery.cookie"
    ...
    $(document).ready(function() {
        ...
        function showPosition(position){ 
            ...
            location.longitude=position.coords.longitude;
                ...
                //Set session
                    setsession(latitude, longitude, custmerloginstatus);
 ...
 // alert($("#lat").val()+'--'+$("#lng").val()+'--'+$.cookie('latitude')+'--'+$.cookie('longitude'));
            ...
            function setsession(latitude, longitude, custmerloginstatus){
                if(latitude != '' && longitude != '' && custmerloginstatus != ''){
                var param = 'latitude='+latitude+'&longitude='+longitude+'&custmerloginstatus='+custmerloginstatus;
                    jQuery.ajax({
                        showLoader: true,
                        url: "http://127.0.0.1/kirana_store/search/index/session",
                        data: param,
                        type: "POST",
                        dataType: 'json',
                    }).done(function (data) {
