...
{
	/** Url Path */
	const GROUP_URL_PATH_EDIT = 'lofmarketplace/payment/edit';
	const GROUP_URL_PATH_DELETE = 'lofmarketplace/payment/delete';
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['payment_id'])) {
                    $item[$name]['edit'] = [
                        'href' => $this->urlBuilder->getUrl($this->editUrl, ['payment_id' => $item['payment_id']]),
                        'label' => __('Edit')
                    ];
                }
            }
