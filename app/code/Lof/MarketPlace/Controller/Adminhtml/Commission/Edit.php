...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Lof_MarketPlace::commission_edit');
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('lof_marketplace_commission', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Seller') : __('New Commission'),
            $id ? __('Edit Seller') : __('New Commission')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Sellers'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getname() : __('New Commission'));
