...
if(jQuery(".sellerowl-play").length > 0 ) {
	var owlItems = [];
	jQuery(".sellerowl-play").each( function(){
		...
		if(!owlItems[jQuery(owlCarousel).attr("id")]){
			...
			if(jQuery("#" + id + " .seller-item").height){
				var height = 0;
				jQuery("#" + id + " .seller-item").each(function(){
					if(jQuery(this).height()>height){
						height = jQuery(this).height();
					}
				});
				if(height>0){
					jQuery("#" + id + " .seller-item").css({"height":height+"px"});
				}
			};
		};
	});
