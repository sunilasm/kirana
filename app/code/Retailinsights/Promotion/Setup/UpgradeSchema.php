...
class UpgradeSchema implements UpgradeSchemaInterface
{
     public function upgrade( SchemaSetupInterface $setup, ModuleContextInterface $context ) {
        ...
        if (version_compare($context->getVersion(), '1.1.6') < 0)  {
            if (!$installer->tableExists('retailinsights_promostoremapp_backup')) {
                $table = $installer->getConnection()->newTable(
                    $installer->getTable('retailinsights_promostoremapp_backup')
                )
                ->addColumn(
                    'p_id',
                    \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                ...
                ->addColumn(
                    ...
                    ->setComment('Promotion Store Mapping backup');
                $installer->getConnection()->createTable($table);
                ...
                $installer->getConnection()->addIndex(
                    ...
                    $setup->getIdxName(
                        $installer->getTable('retailinsights_promostoremapp_backup'),
                        ['store_id','rule','pstart_date','pend_date','status','description','seller_type','conditions_serialized','actions_serialized','simple_action','discount_amount','rule_type'],
                        \Magento\Framework\DB\Adapter\AdapterInterface::INDEX_TYPE_FULLTEXT
                    ),
                    ['store_id','rule','pstart_date','pend_date','status','description','seller_type','conditions_serialized','actions_serialized','simple_action','discount_amount','rule_type'],
