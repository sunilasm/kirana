...
class Interceptor extends \Magento\SalesRule\Model\ResourceModel\Coupon implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function loadPrimaryByRule(\Magento\SalesRule\Model\Coupon $object, $rule)
    {
        ...
        if (!$pluginInfo) {
            return parent::loadPrimaryByRule($object, $rule);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'exists');
        if (!$pluginInfo) {
            return parent::exists($code);
        } else {
            return $this->___callPlugins('exists', func_get_args(), $pluginInfo);
        }
    }
